...
class TestObjectiveImplementations:
    ...
    def test_rastrigin_implementation(self):
        ...
        assert center_result < 1.0, f"Center point should be near minimum: {center_result}"
    ...
    def test_ackley_implementation(self):
        ...
        assert center_result < 1.0, f"Ackley center should be near minimum: {center_result}"

        # Test different dimensions
        for n_dim in [1, 2, 3, 5]:
            center = [0.5] * n_dim
            result = our_ackley_unit_cube(center)
            assert result >= -1e-10, f"Ackley should be non-negative (allowing floating point precision): {result}"
...
class TestOptimizerImplementations:
    ...
    def test_all_optimizers_importable(self):
        ...
        assert len(PURE_OPTIMIZERS) == 22, f"Expected 22 optimizers, got {len(PURE_OPTIMIZERS)}"
    ...
    def test_optimizer_basic_functionality(self):
        ...
        from humpday.optimizers.optimizers import RandomSearch, NelderMead
        ...
        assert rs_best_val < 1.0, f"RandomSearch didn't find good solution: {rs_best_val}"
        assert len(rs_best_x) == 2, f"Wrong solution dimension: {len(rs_best_x)}"
        assert rs_optimizer.evaluations <= 100, f"Too many evaluations: {rs_optimizer.evaluations}"
    ...
    def test_optimizers_respect_bounds(self):
        ...
        test_optimizers = ['RandomSearch', 'NelderMead', 'DifferentialEvolution', 'ParticleSwarm']
    ...
    def test_optimizer_consistency(self):
        ...
        assert abs(result1[0] - result2[0]) < 1e-10, "Non-deterministic behavior with same seed"
