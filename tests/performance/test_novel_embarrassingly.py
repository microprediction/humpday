...
class NovelTechniqueAnalysis:
    ...
    def create_expensive_functions(self) -> Dict[str, Callable]:
        ...
        def expensive_sphere(x):
            ...
            compute_time = 0.001 + 0.02 * np.exp(
                -10 * boundary_penalty
            )  # Exponential cost near boundaries
        ...
        def expensive_rosenbrock(x):
            ...
            if len(scaled_x) >= 2:
                grad_approx = abs(
                    400 * scaled_x[0] * (scaled_x[0] ** 2 - scaled_x[1])
                    + 2 * (scaled_x[0] - 1)
                )
                compute_time = 0.001 + 0.01 * min(
                    grad_approx / 1000, 0.5
    ...
    def run_optimization_battery(
        self, optimizer_name: str, objective_func: Callable, function_name: str
    ) -> List[Dict]:
        ...
        np.random.seed(42)
        random_starts = [
            np.random.rand(2).tolist() for _ in range(self.n_runs - len(start_points))
        ]
    ...
    def test_shy_adaptive_evaluation(self):
        ...
        print("=" * 50)
        print(
            "Hypothesis: Shy should save time on expensive functions while maintaining solution quality"
        )
        ...
        for func_name, func in expensive_functions.items():
            ...
            for optimizer_name in optimizers:
                ...
                standard_results = self.run_optimization_battery(
                    optimizer_name, func, func_name
                )
                ...
                std_avg_eval_time = np.mean(
                    [r["avg_eval_time"] for r in successful_standard]
                )

                print(
                    f"✓ {len(successful_standard)}/{len(standard_results)} successful"
                )
                print(
                    f"      Value: {std_avg_value:.4f}, Total time: {std_avg_time:.3f}s, Eval time: {std_avg_eval_time:.4f}s"
                )
                ...
                for shy_config in shy_configs:
                    shy_func = Shy(
                        func,
                        bounds=bounds,
                        **{k: v for k, v in shy_config.items() if k != "name"},
                    )

                    print(f"    Shy {shy_config['name']}...    ", end="", flush=True)
                    shy_results = self.run_optimization_battery(
                        optimizer_name, shy_func, f"{func_name}_shy"
                    )
                    ...
                    shy_avg_eval_time = np.mean(
                        [r["avg_eval_time"] for r in successful_shy]
                    )
                    ...
                    value_improvement = (
                        (std_avg_value - shy_avg_value) / std_avg_value * 100
                    )
                    time_improvement = (
                        (std_avg_time - shy_avg_time) / std_avg_time * 100
                    )

                    print(f"✓ {len(successful_shy)}/{len(shy_results)} successful")
                    print(
                        f"      Value: {shy_avg_value:.4f} ({value_improvement:+.1f}%, p={value_p:.3f})"
                    )
                    print(
                        f"      Time:  {shy_avg_time:.3f}s ({time_improvement:+.1f}%, p={time_p:.3f})"
                    )
        ...
        if len(df) > 0:
            ...
            sig_value_improvements = df[
                (df["value_p_value"] < 0.05) & (df["value_improvement"] > 0)
            ]
            sig_time_improvements = df[
                (df["time_p_value"] < 0.05) & (df["time_improvement"] > 0)
            ]

            print(
                f"Significant value improvements: {len(sig_value_improvements)}/{len(df)}"
            )
            print(
                f"Significant time improvements: {len(sig_time_improvements)}/{len(df)}"
            )

            if len(sig_value_improvements) > 0:
                print(
                    f"Best value improvement: {sig_value_improvements['value_improvement'].max():.1f}%"
                )
            if len(sig_time_improvements) > 0:
                print(
                    f"Best time improvement: {sig_time_improvements['time_improvement'].max():.1f}%"
                )

            # Overall verdict on Shy
            success_rate = (
                len(sig_value_improvements) + len(sig_time_improvements)
            ) / len(df)
            if success_rate > 0.3:
    ...
    def test_underpromoted_plateau_finding(self):
        ...
        print("=" * 55)
        print(
            "Hypothesis: Underpromoted should prefer stable plateau regions over sharp minima"
        )
        ...
        for landscape_name, landscape_func in plateau_landscapes.items():
            ...
            for optimizer_name in optimizers:
                ...
                print("    Standard version... ", end="", flush=True)
                standard_results = self.run_optimization_battery(
                    optimizer_name, landscape_func, landscape_name
                )
                successful_standard = [r for r in standard_results if r["success"]]
                ...
                for radius in radius_configs:
                    plateau_func = Underpromoted2d(
                        landscape_func, bounds=bounds, radius=radius
                    )

                    print(f"    Plateau radius {radius:.2f}... ", end="", flush=True)
                    plateau_results = self.run_optimization_battery(
                        optimizer_name, plateau_func, f"{landscape_name}_plateau"
                    )
                    ...
                    value_improvement = (
                        (std_avg_value - plateau_avg_value) / std_avg_value * 100
                    )

                    print(
                        f"✓ {len(successful_plateau)}/{len(plateau_results)} successful"
                    )
                    print(
                        f"      Average value: {plateau_avg_value:.4f} ({value_improvement:+.1f}%, p={value_p:.3f})"
                    )
        ...
        if len(df) > 0:
            ...
            if len(sig_improvements) > 0:
                print(
                    f"Best improvement: {sig_improvements['value_improvement'].max():.1f}%"
                )
                best_config = sig_improvements.loc[
                    sig_improvements["value_improvement"].idxmax()
                ]
                print(
                    f"Best config: {best_config['landscape']} with {best_config['optimizer']} (radius={best_config['radius']:.2f})"
                )
            ...
            if success_rate > 0.3:
                ...
                print(
                    "🤔 VERDICT: Underpromoted shows occasional plateau-finding benefits"
                )
            else:
                print(
                    "❌ VERDICT: Underpromoted doesn't consistently find better plateaus"
                )
...
if __name__ == "__main__":
    ...
    print(
        "measurable, statistically significant benefits for derivative-free optimization."
    )
