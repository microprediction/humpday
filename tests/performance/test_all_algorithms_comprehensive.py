...
class ComprehensiveAlgorithmValidator:
    ...
    def __init__(self):
        ...
        self.test_functions = {
            ...
            'beale2d': {
                'name': '2D Beale',
                'python_func': lambda x: (1.5 - x[0] + x[0]*x[1])**2 + (2.25 - x[0] + x[0]*x[1]**2)**2 + (2.625 - x[0] + x[0]*x[1]**3)**2,
                'js_func': 'x => { const a = 1.5 - x[0] + x[0]*x[1]; const b = 2.25 - x[0] + x[0]*x[1]*x[1]; const c = 2.625 - x[0] + x[0]*x[1]*x[1]*x[1]; return a*a + b*b + c*c; }',
                'optimum': [3, 0.5],
                'optimum_value': 0,
                'dimensions': 2,
                'bounds': [(0, 1), (0, 1)]  # Note: true optimum outside [0,1], so this tests constrained optimization
            }
        }
    ...
    def _test_pdfo_uobyqa(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = uobyqa(test_func['python_func'], x0,
    ...
    def _test_pdfo_newuoa(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = newuoa(test_func['python_func'], x0,
                          options={'maxfev': max_evals, 'rhobeg': 0.1, 'rhoend': 1e-6})
    ...
    def _test_pdfo_bobyqa(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = bobyqa(test_func['python_func'], x0, bounds=bounds,
                          options={'maxfev': max_evals, 'rhobeg': 0.1, 'rhoend': 1e-6})
    ...
    def _test_scipy_nelder_mead(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = minimize(test_func['python_func'], x0, method='Nelder-Mead',
                            bounds=bounds, options={'maxfev': max_evals})
    ...
    def _test_scipy_differential_evolution(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = differential_evolution(test_func['python_func'], test_func['bounds'],
                                          maxiter=max_evals//10, seed=42)
    ...
    def _test_scipy_simulated_annealing(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = dual_annealing(test_func['python_func'], test_func['bounds'],
                                  maxfun=max_evals, seed=42)
    ...
    def _test_deap_genetic_algorithm(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            toolbox.register("individual", tools.initRepeat, creator.Individual,
                           toolbox.attr_float, n=test_func['dimensions'])
    ...
    def _test_deap_evolution_strategy(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            def eval_func(individual):
                # Clip to bounds
                individual = np.clip(individual, [b[0] for b in test_func['bounds']], [b[1] for b in test_func['bounds']])
    ...
    def _test_pyswarm_pso(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            # Run PSO
            xopt, fopt = pso(test_func['python_func'], lb, ub, maxiter=max_evals//20, debug=False)
    ...
    def _test_cma_es(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            # Initialize
            x_mean = np.array([0.5 * (b[0] + b[1]) for b in bounds])  # Center of search space
            sigma = 0.3  # Step size
    ...
    def _test_skopt_bayesian(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = gp_minimize(wrapped_func, dimensions,
                               n_calls=max_evals, random_state=42,
    ...
    def _test_scipy_bfgs(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            result = minimize(test_func['python_func'], x0, method='BFGS',
                            bounds=bounds, options={'maxiter': max_evals})
    ...
    def _test_adaptive_random_search_external(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            try:
                ...
                opt = nlopt.opt(nlopt.GN_CRS2_LM, len(bounds))  # Controlled Random Search
    ...
    def _test_custom_hill_climbing(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            return {'success': True, 'x': current_x.tolist(), 'fun': float(current_f), 'nfev': max_evals}
        except Exception as e:
            return {'error': str(e)}
        ...
        try:
            ...
            for i in range(max_evals - 1):
                ...
                for _ in range(min(10, max_evals - i)):
                    ...
                    is_tabu = any(np.linalg.norm(neighbor - tabu_x) < 0.01 for tabu_x in tabu_list)
                    if not is_tabu:
            ...
            return {'success': True, 'x': best_x.tolist(), 'fun': float(best_f), 'nfev': max_evals}
        except Exception as e:
            return {'error': str(e)}
    ...
    def _test_custom_firefly_algorithm(self, func_name: str, max_evals: int = 100):
        ...
        try:
            ...
            np.random.seed(42)
            n_fireflies = 10
            fireflies = np.random.uniform([b[0] for b in bounds], [b[1] for b in bounds],
            ...
            for i in range(max_evals // n_fireflies):
                ...
                for j in range(n_fireflies):
                    if j != best_idx:
                        ...
                        fireflies[j] += 0.1 * direction + np.random.normal(0, 0.01, len(bounds))
                        fireflies[j] = np.clip(fireflies[j], [b[0] for b in bounds], [b[1] for b in bounds])

            return {'success': True, 'x': fireflies[best_idx].tolist(), 'fun': float(best_f), 'nfev': max_evals}
        except Exception as e:
            return {'error': str(e)}
        ...
        try:
            ...
            try:
                ...
                class TestObjective(ObjectiveFunction):
                    ...
                    def use_vars(self):
                        return [{'name': f'x{i}', 'type': 'float', 'lower_bound': b[0], 'upper_bound': b[1]}
                ...
                result = harmony_search(obj_fun, max_evals=max_evals, hms=10, hmcr=0.9, par=0.3)
    ...
    def run_js_optimization(self, algorithm: str, func_name: str, max_evals: int = 100) -> Dict[str, Any]:
...
"""
    ...
    def run_js_optimization_multiple_runs(self, algorithm: str, func_name: str, num_runs: int = 20, max_evals: int = 100):
...
"""
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    results.append({'error': f'JavaScript execution failed: {result.stderr}', 'run_id': run})

            except subprocess.TimeoutExpired:
                results.append({'error': 'JavaScript execution timed out', 'run_id': run})
            except Exception as e:
    ...
    def test_algorithm_comprehensive(self, algorithm: str, max_evals: int = 100) -> AlgorithmTestResult:
        """Test one algorithm comprehensively against its reference"""
        if algorithm not in self.algorithms:
            return AlgorithmTestResult(
                algorithm=algorithm, package='UNKNOWN', js_success=False, ref_success=False,
                perfect_matches=0, total_tests=0, avg_js_value=float('inf'), avg_ref_value=float('inf'),
                max_difference=float('inf'), passed_validation=False,
                error_message='Algorithm not found', win_rate_vs_ref=0.0
            )
        ...
        for func_name, test_func in self.test_functions.items():
            ...
            js_multi_results = self.run_js_optimization_multiple_runs(js_name, func_name, num_runs=20, max_evals=max_evals)
            ...
            if js_perfect_matches >= 5:  # At least 25% perfect matches
                ...
                js_wins += 1
                print(f"⚡ JS WINS ({win_rate_this_func:.1%} win rate, best: {js_best_value:.6f})")
            else:
                print(f"⚠️  REF WINS ({win_rate_this_func:.1%} win rate, ref: {ref_value:.6f})")

        # Calculate metrics
        avg_js = np.mean(js_results) if js_results else float('inf')
        avg_ref = np.mean(ref_results) if ref_results else float('inf')
        max_diff = max([abs(j-r) for j,r in zip(js_results, ref_results)]) if js_results and ref_results else float('inf')
        win_rate = (js_wins + perfect_matches) / total_tests if total_tests > 0 else 0.0

        # Validation criteria
        passed = (perfect_matches >= 1) or (win_rate >= 0.4)  # At least 1 perfect match OR 40%+ win rate
    ...
    def run_comprehensive_validation(self, priority_filter=None) -> Dict[str, AlgorithmTestResult]:
        ...
        if priority_filter:
            algorithms_to_test = [algo for algo in algorithms_to_test
    ...
    def generate_comprehensive_report(self, results: Dict[str, AlgorithmTestResult]) -> str:
        ...
        perfect_match_algorithms = sum(1 for r in results.values() if r.perfect_matches > 0)

        report.append(f"## 📊 SUMMARY\n")
        report.append(f"- **Total Algorithms Tested**: {total_algorithms}")
        report.append(f"- **Passed Validation**: {passed_algorithms}/{total_algorithms} ({100*passed_algorithms/total_algorithms:.1f}%)")
        report.append(f"- **Have Perfect Matches**: {perfect_match_algorithms}/{total_algorithms} ({100*perfect_match_algorithms/total_algorithms:.1f}%)")
        report.append("")

        # Group by validation status
        excellent = [a for a, r in results.items() if r.perfect_matches >= 2]
        good = [a for a, r in results.items() if r.perfect_matches == 1 or r.win_rate_vs_ref >= 0.5]
        needs_work = [a for a, r in results.items() if r.perfect_matches == 0 and r.win_rate_vs_ref < 0.5]

        report.append("## 🎯 ALGORITHM STATUS\n")
        report.append("```")
        report.append(f"EXCELLENT (✅): {', '.join(excellent) if excellent else 'None'}")
        report.append(f"GOOD (⚠️):      {', '.join(good) if good else 'None'}")
        report.append(f"NEEDS WORK (❌): {', '.join(needs_work) if needs_work else 'None'}")
        report.append("```\n")
        ...
        for algorithm, result in results.items():
            status = "✅ EXCELLENT" if result.perfect_matches >= 2 else ("⚠️ GOOD" if result.perfect_matches >= 1 or result.win_rate_vs_ref >= 0.5 else "❌ NEEDS WORK")

            report.append(f"### {algorithm} ({result.package}) - {status}\n")
            report.append(f"- **Perfect Matches**: {result.perfect_matches}/{result.total_tests}")
...
def main():
    ...
    if test_all:
        ...
        results = validator.run_comprehensive_validation()  # No priority filter = all algorithms
    else:
    ...
    print(f"📊 Raw results saved to: comprehensive_algorithm_results.json")
