...
from humpday.objectives.stochastic_surfaces import StochasticSurfaceGenerator, create_fair_benchmark_run
from humpday.comparison.thurstone_ratings import OptimizerThurstoneCaliber
...
def run_single_comparison(optimizer_func, objective_func, objective_name: str, n_dim: int, n_trials: int, seed: int = None) -> Dict:
...
def run_benchmark_suite(
    ...
    total_runs = len(optimizers) * total_functions * len(n_dims) * len(n_trials_list) * n_repeats
    current_run = 0
    ...
    for repeat in range(n_repeats):
        ...
        for obj_type in objective_types:
            for func_idx in range(n_functions_per_type):
                ...
                if hasattr(surface_generator, f'stochastic_{obj_type}'):
                    stochastic_func = getattr(surface_generator, f'stochastic_{obj_type}')
                    function_suite[instance_name] = stochastic_func(function_id=instance_name)
                else:
        ...
        print(f"   Surface parameters: shift={metadata['global_shift']:.3f}, "
            ...
            for n_dim in n_dims:
                for n_trials in n_trials_list:
                    ...
                    for optimizer in optimizers:
                        ...
                        run_seed = hash(f"{optimizer.__name__}_{func_name}_{n_dim}_{n_trials}_{repeat}") % 2**31

                        result = run_single_comparison(
                            optimizer, objective_func, func_name, n_dim, n_trials, run_seed
                        )
                        result['repeat'] = repeat
                        result['surface_seed'] = repeat_seed  # Track which surface set this came from
                        result['run_seed'] = run_seed
                        result['surface_type'] = func_name.split('_')[0]  # sphere, rastrigin, etc.
    ...
    with open(metadata_path, 'w') as f:
        ...
        f.write(f"Random surface generation: Enabled\n")
        f.write(f"Surface parameters varied: shift, rotation, scale, noise, conditioning\n")
        f.write(f"\nCritical: Each repeat used completely different random surfaces\n")
        f.write(f"to prevent bias from lucky/unlucky initial guesses or fixed landscapes.\n")
...
def compute_thurstone_rankings(df: pd.DataFrame, output_dir: str = "results") -> Dict:
    ...
    print(f"Successful runs: {len(successful)}/{len(df)} ({100*len(successful)/len(df):.1f}%)")
    ...
    optimizer_avg_performance = successful.groupby('optimizer')['best_value'].mean().sort_values()
    ...
    contexts = [
        ('dimension', ['n_dim']),
        ('trials', ['n_trials']),
        ('surface_type', ['surface_type'])  # Group by surface type (sphere, rastrigin, etc)
    ]
    ...
    for context_name, group_cols in contexts:
        ...
        for group_values, group_data in successful.groupby(group_cols):
            ...
            group_key = group_values if isinstance(group_values, str) else '_'.join(map(str, group_values))

            # Simple average performance ranking for this context
            context_avg = group_data.groupby('optimizer')['best_value'].mean().sort_values()
...
def generate_summary_report(df: pd.DataFrame, rankings: Dict, output_dir: str = "results") -> str:
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"- **Methodology**: Stochastic surface generation (random surfaces per run)\n")
        f.write(f"- Total runs: {len(df)}\n")
        f.write(f"- Successful runs: {len(successful)} ({100*len(successful)/len(df):.1f}%)\n")
        f.write(f"- Unique optimizers: {df['optimizer'].nunique()}\n")
        f.write(f"- Surface types: {sorted(df['surface_type'].unique()) if 'surface_type' in df.columns else 'N/A'}\n")
        f.write(f"- Unique surface instances: {df['surface_instance'].nunique() if 'surface_instance' in df.columns else 'N/A'}\n")
        f.write(f"- Dimensions tested: {sorted(df['n_dim'].unique())}\n")
        f.write(f"- Trial counts: {sorted(df['n_trials'].unique())}\n")
        f.write(f"- Benchmark repeats: {df['repeat'].nunique() if 'repeat' in df.columns else 'N/A'}\n\n")

        f.write("## Stochastic Surface Generation\n\n")
        f.write("**Critical Innovation**: Each comparison run used completely random surfaces to prevent bias.\n")
        f.write("- Random shifts, rotations, scaling, and noise applied to base functions\n")
        f.write("- No algorithm could benefit from memorizing fixed landscapes\n")
        f.write("- Fair comparison based on true algorithmic capability, not lucky guesses\n\n")

        f.write("## Optimizer Success Rates\n\n")
        success_rates = df.groupby('optimizer')['success'].agg(['count', 'sum', 'mean']).round(3)
        ...
        best_values = successful.groupby('optimizer')['best_value'].agg(['mean', 'std', 'min', 'median']).round(4)
        best_values = best_values.sort_values('mean')
        f.write(best_values.to_string())
        f.write("\n\n")

        f.write("### Runtime Statistics (seconds)\n")
        runtime_stats = successful.groupby('optimizer')['elapsed_seconds'].agg(['mean', 'std', 'min', 'median']).round(3)
...
def main():
    ...
    results_df = run_benchmark_suite(
        ...
        objective_types=['sphere', 'rastrigin', 'rosenbrock', 'ackley'],  # Stochastic surface types
    ...
    print(f"- {results_df['surface_type'].nunique() if 'surface_type' in results_df.columns else 'N/A'} surface types")
    print(f"- {results_df['surface_instance'].nunique() if 'surface_instance' in results_df.columns else 'N/A'} unique random surfaces")
    print(f"- {results_df['repeat'].nunique() if 'repeat' in results_df.columns else 'N/A'} benchmark repeats (each with fresh surfaces)")
    print(f"✅ Every comparison used different random surfaces - no bias from fixed landscapes!")
