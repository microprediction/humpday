...
def test_plateau_finding():
    ...
    for method in methods:
        ...
        standard_avg = np.mean([r['final_value'] for r in standard_results if r['success']])

        # Plateau-enhanced optimization
        plateau_landscape = Underpromoted2d(simple_landscape, bounds=bounds, radius=0.08)
        plateau_results = run_optimization_test(plateau_landscape, method)
        plateau_avg = np.mean([r['final_value'] for r in plateau_results if r['success']])

        improvement = (standard_avg - plateau_avg) / standard_avg * 100 if standard_avg != 0 else 0
    ...
    for method in methods:
        ...
        standard_analysis = analyze_plateau_preference(standard_results, complex_landscape)
        ...
        plateau_analysis = analyze_plateau_preference(plateau_results, complex_landscape)

        # Count preferences
        standard_plateau_count = sum(1 for a in standard_analysis if 'plateau' in a['region'])
        standard_sharp_count = sum(1 for a in standard_analysis if a['region'] == 'sharp_minimum')

        plateau_plateau_count = sum(1 for a in plateau_analysis if 'plateau' in a['region'])
        plateau_sharp_count = sum(1 for a in plateau_analysis if a['region'] == 'sharp_minimum')

        print(f"  Standard optimization:")
        print(f"    Found plateaus: {standard_plateau_count}/{len(standard_analysis)}")
        print(f"    Found sharp minimum: {standard_sharp_count}/{len(standard_analysis)}")

        print(f"  Plateau-enhanced optimization:")
        ...
        if plateau_plateau_count > standard_plateau_count:
            print(f"  🏆 Underpromoted prefers robust plateau regions!")
        elif plateau_sharp_count < standard_sharp_count:
            print(f"  🛡️ Underpromoted avoids fragile sharp minima!")
        else:
            print(f"  ≈ No clear preference change")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Check results above to see if Underpromoted helps find robust solutions.")
