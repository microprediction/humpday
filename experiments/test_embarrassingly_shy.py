...
from humpday.optimizers.scipycube import scipy_powell_cube, scipy_slsqp_cube, scipy_lbfgsb_cube
...
def run_optimizer_test(optimizer_func, objective_func, n_trials: int = 25, n_dim: int = 2) -> Dict:
...
def test_shy_vs_standard():
    ...
    for func_name, expensive_func in expensive_functions:
        ...
        for opt_name, optimizer in optimizers:
            ...
            standard_result = run_optimizer_test(optimizer, expensive_func, n_trials, n_dim)
            print(f"✓ {standard_result['best_value']:.4f} in {standard_result['elapsed_time']:.2f}s")

            # Test with Shy wrapper
            print("    Shy evaluation...     ", end=" ", flush=True)
            shy_result = run_optimizer_test(optimizer, shy_func, n_trials, n_dim)
            print(f"✓ {shy_result['best_value']:.4f} in {shy_result['elapsed_time']:.2f}s")

            # Calculate improvements
            time_improvement = (standard_result['elapsed_time'] - shy_result['elapsed_time']) / standard_result['elapsed_time']
            value_improvement = (standard_result['best_value'] - shy_result['best_value']) / abs(standard_result['best_value']) if standard_result['best_value'] != 0 else 0
            ...
            if abs(value_improvement) > 0.1:  # More than 10% different
                if value_improvement > 0:
                    print(f"      🎯 Shy finds {value_improvement*100:.1f}% better solution!")
                else:
                    print(f"      📉 Shy finds {abs(value_improvement)*100:.1f}% worse solution")
...
def test_plateau_finding():
    ...
    for opt_name, optimizer in optimizers:
        ...
        improvement = (standard_result['best_value'] - plateau_result['best_value']) / standard_result['best_value']

        if improvement > 0.05:
            print(f"      🎯 Underpromoted found {improvement*100:.1f}% better solution!")
        elif improvement < -0.05:
            print(f"      📉 Underpromoted found {abs(improvement)*100:.1f}% worse solution")
        else:
            print(f"      ≈ Similar performance")
...
if __name__ == "__main__":
    ...
    print(f"\n\n🏁 FINAL CONCLUSIONS")
    print("=" * 80)
    print("✅ Tests completed - check results above to see if embarrassingly techniques work!")
