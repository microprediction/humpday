...
def run_single_comparison(
    optimizer_func, objective_func, n_dim: int, n_trials: int, seed: int = None
) -> Dict:
...
def run_comprehensive_benchmark():
    ...
    total_runs = (
        len(optimizers) * len(objectives) * len(n_dims) * len(n_trials_list) * n_repeats
    )
    ...
    if total_runs > 1500:
        ...
        print(
            f"Reduced to {total_runs} runs (estimated ~{total_runs / 60:.1f} minutes)"
        )
            ...
            for budget_idx, n_trials in enumerate(n_trials_list):
                ...
                for repeat in range(n_repeats):
                    for opt_idx, optimizer in enumerate(optimizers):
                        ...
                        seed = hash(
                            f"{optimizer.__name__}_{objective.__name__}_{n_dim}_{n_trials}_{repeat}"
                        ) % (2**31)
                        ...
                        if current_run % (len(optimizers) * n_repeats) == 0:
                            success_rate = (
                                (current_run - failed_runs) / current_run * 100
                            )
...
def create_comprehensive_3d_analysis(df):
    ...
    print(
        f"Analyzing {n_successful}/{n_total} successful runs ({100 * n_successful / n_total:.1f}%)"
    )
    ...
    landscape_mapping = {
        ...
        "zakharov": "smooth",
        "bohachevsky": "smooth",
        "rotated_hyper_ellipsoid": "smooth",
        "rastrigin": "multimodal",
        "griewank": "multimodal",
        "ackley": "multimodal",
        "styblinski_tang": "multimodal",
        "schwefel": "multimodal",
        "shekel": "rugged",
        "shaffer": "rugged",
        "deap_combo1": "rugged",
    ...
    def budget_category(budget_per_dim):
        if budget_per_dim < 15:
            return "low"
        elif budget_per_dim < 30:
            return "medium"
        else:
            return "high"
    ...
    for optimizer in optimizers:
        perf_matrix[optimizer] = [
            tensor_data[optimizer][key]["performance"] for key in context_keys
        ]
    ...
    consistency = (1 / (tensor_df.std(axis=1) + 0.01)).sort_values(
        ascending=False
    ...
    specialization = (tensor_df.max(axis=1) - tensor_df.min(axis=1)).sort_values(
        ascending=False
    )

    print("Overall Performance Leaders:")
    ...
    analysis_results = {
        "performance_tensor": {
            opt: {k: v["performance"] for k, v in profile.items()}
...
def main():
    ...
    print("✓ Comprehensive multi-dimensional Thurstone calibration complete")
    print("✓ Context-specific recommendations generated")
    print(
        f"✓ Rich performance tensor with {len(tensor_df)} optimizers across {len(tensor_df.columns)} contexts"
    )
    print("✓ Statistical confidence metrics included")
    print("✓ Ready for JSS paper results section")
