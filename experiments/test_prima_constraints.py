...
def test_constraint_handling():
    ...
    for method_name, method_func in methods.items():
        ...
        for run in range(3):
            ...
            closest_to_optimum = np.linalg.norm(x - np.array([1.0, 1.0]))  # [1,1] is closest feasible point

            print(f"  Run {run+1}: x = [{x[0]:.3f}, {x[1]:.3f}], f = {val:.4f}, "
    ...
    for method_name, method_func in methods.items():
        ...
        in_bounds = np.all((x >= 0.0) & (x <= 1.0))
        print(f"  {method_name}: x = [{x[0]:.4f}, {x[1]:.4f}], f = {val:.6f}, "
