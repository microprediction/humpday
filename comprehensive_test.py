...
class HumpDayTester:
                ...
                try:
                    ...
                    if optimizer_func:
                        ...
                        best_val, best_x = optimizer_func(
                            sphere_function, n_dim=2, n_trials=10
                        )
        ...
        for file_path, (min_size, max_size) in required_files.items():
            ...
            if full_path.exists():
                ...
                if min_size <= size <= max_size:
                    ...
                    print(
                        f"  ⚠ {file_path} ({size / 1024:.1f} KB) - Size outside expected range"
                    )
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    print(
                        f"✓ Server started successfully on http://127.0.0.1:{self.server_port}"
                    )
                    return True
                else:
    ...
    def test_external_links(self):
        ...
        for i, link in enumerate(
            all_links[:20], 1
        ):  # Test first 20 to avoid rate limits
            try:
    ...
    def generate_report(self):
        ...
        if "python_tests" in self.results and "working" in self.results["python_tests"]:
            ...
            print(
                f"🐍 Python: {py['working']}/{py['total_algorithms']} algorithms working"
            )
        ...
        # Website summary
        if "website_tests" in self.results and isinstance(
            self.results["website_tests"], dict
        ):
        ...
        if "link_tests" in self.results and "working" in self.results["link_tests"]:
            ...
            print(
                f"🔗 Links: {links['working']}/{links['tested']} external links working"
            )
