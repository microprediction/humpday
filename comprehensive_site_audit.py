...
class ComprehensiveSiteAuditor:
    ...
    def test_all_python_algorithms(self):
        ...
        try:
            ...
            test_functions = {
                ...
                "rosenbrock": lambda x: sum(
                    100.0 * (x[i + 1] - x[i] ** 2) ** 2 + (1 - x[i]) ** 2
                    for i in range(len(x) - 1)
                ),
                "rastrigin": lambda x: (
                    10 * len(x) + sum(xi**2 - 10 * np.cos(2 * np.pi * xi) for xi in x)
                ),
            }
            ...
            for alg_name in sorted(PURE_OPTIMIZERS.keys()):
                ...
                try:
                    ...
                    if not optimizer_func:
                        algorithm_results[alg_name]["status"] = (
                            "failed_to_get_optimizer"
                        )
                        continue
                        ...
                        try:
                            ...
                            for n_dim in [2, 5, 10]:
                                ...
                                best_val, best_x = optimizer_func(
                                    test_func, n_dim=n_dim, n_trials=50
                                )
                                end_time = time.time()
                                ...
                                print(
                                    f"  {test_key}: {best_val:.6f} ({(end_time - start_time) * 1000:.1f}ms)"
                                )
    ...
    def start_test_server(self):
        ...
        try:
            ...
            subprocess.run(
                ["pkill", "-f", f"http.server {self.server_port}"],
                capture_output=True,
                cwd=self.docs_path,
            )
            time.sleep(1)
            ...
            if response.status_code == 200:
                print(
                    f"Server started successfully: http://127.0.0.1:{self.server_port}"
                )
                return True
            else:
    ...
    def test_page_performance(self):
        ...
        for page in pages:
            ...
            try:
                ...
                print(
                    f"  Status: {response.status_code}, Size: {len(response.content) / 1024:.1f}KB, Time: {load_time * 1000:.0f}ms"
                )

            except Exception as e:
                performance_results[page] = {"error": str(e), "success": False}
                print(f"  ERROR: {e}")
        ...
        for module in js_modules:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"  OK: {class_count} classes, {function_count} functions, {len(content) / 1024:.1f}KB"
                    )
                else:
                    js_results[module] = {
                        "status_code": response.status_code,
                ...
                js_results[module] = {"error": str(e), "success": False}
                print(f"  ERROR: {e}")
    ...
    def comprehensive_link_testing(self):
        ...
        for link in list(internal_links)[:20]:  # Test first 20
            try:
                ...
                internal_results[link] = {"error": str(e), "success": False}
        ...
        for link in external_sample:
            try:
                ...
                external_results[link] = {"error": str(e), "success": False}
    ...
    def audit_style_consistency(self):
        ...
        if style_audit["emoji_usage"]:
            ...
            for emoji_file in style_audit["emoji_usage"]:
                print(
                    f"  {emoji_file['file']}: {emoji_file['emojis']} ({emoji_file['count']} emojis)"
                )
        ...
        for html_file in html_files:
            try:
                ...
                generic_links = [
                    ...
                    if text.strip().lower() in ["click here", "here", "link", "more"]
                ]
                if generic_links:
                ...
                accessibility_results[html_file.name] = {"error": str(e)}
    ...
    def generate_comprehensive_report(self):
        ...
        if "python" in self.results["algorithm_tests"]:
            ...
            working_algs = [
                name
                for name, data in py_results.items()
        ...
        if "javascript" in self.results["algorithm_tests"]:
            ...
            working_modules = sum(
                1 for module in js_results.values() if module.get("success", False)
            )
            print(
                f"JavaScript Modules: {working_modules}/{len(js_results)} loading correctly"
            )
        ...
        if "performance_tests" in self.results:
            ...
            working_pages = sum(
                1 for page in perf_results.values() if page.get("success", False)
            )
            avg_load_time = sum(
                page.get("load_time_ms", 0)
                for page in perf_results.values()
                if "load_time_ms" in page
            ) / len(perf_results)
            print(
                f"Page Performance: {working_pages}/{len(perf_results)} pages, avg load: {avg_load_time:.0f}ms"
            )
        ...
        if "link_tests" in self.results:
            ...
            internal_ok = sum(
                1
                for link in link_results["internal"].values()
                if link.get("success", False)
            )
            external_ok = sum(
                1
                for link in link_results["external"].values()
                if link.get("success", False)
            )
            print(
                f"Link Testing: {internal_ok}/{len(link_results['internal'])} internal, {external_ok}/{len(link_results['external'])} external working"
            )
        ...
        if "style_audit" in self.results:
            ...
            print(
                f"Style Audit: {emoji_files} files with emojis (NEED FIXING), {font_issues} font inconsistencies"
            )
        ...
        if "accessibility_tests" in self.results:
            ...
            total_issues = sum(
                ...
                for result in acc_results.values()
                if isinstance(result, dict)
            )
            print(
                f"Accessibility: {total_issues} issues found across {len(acc_results)} pages"
            )
