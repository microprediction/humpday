...
class ProblemCharacteristics:
    ...
    conditioning: str        # well_conditioned, moderate, ill_conditioned, very_ill_conditioned
...
class AdvancedProblemCategorizer:
    ...
    def categorize_function(self, function_name: str, n_dim: int,
                          n_trials: int, metadata: Dict = None) -> ProblemCharacteristics:
        ...
        # Meta characteristics
        difficulty = self._infer_difficulty(landscape_type, conditioning,
                                          modality_density, global_structure, n_dim)
    ...
    def _infer_landscape_type(self, function_name: str, metadata: Dict) -> str:
        ...
        if any(smooth in name_lower for smooth in ['sphere', 'ellipsoid', 'rosenbrock', 'zakharov']):
            return 'smooth'
        elif any(rugged in name_lower for rugged in ['schwefel', 'gallagher', 'composition']):
            return 'rugged'
        else:
            return 'multimodal'
    ...
    def _infer_separability(self, function_name: str, metadata: Dict) -> str:
        ...
        name_lower = function_name.lower()
        if 'sep' in name_lower or any(sep in name_lower for sep in ['sphere', 'rastrigin']):
            return 'separable'
    ...
    def _infer_difficulty(self, landscape_type: str, conditioning: str,
                         modality_density: str, global_structure: str, n_dim: int) -> str:
        ...
        conditioning_scores = {
            'well_conditioned': 0, 'moderate': 1,
            'ill_conditioned': 2, 'very_ill_conditioned': 3
        }
        difficulty_score += conditioning_scores.get(conditioning, 1)

        # Modality contribution
        modality_scores = {
            'unimodal': 0, 'multimodal': 1,
            'highly_multimodal': 2
        }
...
class Advanced5DThurstonAnalyzer:
    ...
    def analyze_benchmark_results(self, results_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        context_columns = [
            'landscape_type', 'dimensionality', 'budget_class',
            'conditioning', 'separability', 'global_structure'
        ]

        # Group by specific problem instances
        problem_groups = enhanced_df.groupby(['objective', 'n_dim', 'n_trials', 'repeat'])
        ...
        return {
            ...
            'advanced_recommendations': self._generate_advanced_recommendations(performance_analysis),
            'categorization_insights': self._analyze_categorization_patterns(enhanced_df)
        }
                        ...
                        for conditioning in conditionings:
                            for separability in separabilities:
                                ...
                                if len(context_data) > 0:
                                    profile[context_key] = {
                                        'performance': context_data['relative_performance'].mean(),
                                        'std': context_data['relative_performance'].std(),
    ...
    def _generate_advanced_recommendations(self, tensor_analysis: Dict) -> Dict[str, str]:
        ...
        for context_key in list(tensor_data.values())[0].keys():
            ...
            if context_scores:
                best_optimizer = max(context_scores.keys(), key=lambda x: context_scores[x])
                best_score = context_scores[best_optimizer]

                if best_score > 0.6:  # Only recommend if clearly better
                    recommendations[context_key] = f"{best_optimizer} ({best_score:.3f})"
    ...
    def _analyze_categorization_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'separability_performance': df.groupby('separability')['relative_performance'].mean().to_dict(),
            'conditioning_performance': df.groupby('conditioning')['relative_performance'].mean().to_dict(),
            'structure_performance': df.groupby('global_structure')['relative_performance'].mean().to_dict()
        }
...
if __name__ == "__main__":
    ...
    print(f"✓ Advanced multi-dimensional categorization")
    print(f"✓ BBOB-inspired systematic approach")
    print(f"✓ Sophisticated context-specific recommendations")
    print(f"✓ Research-backed problem characteristics")