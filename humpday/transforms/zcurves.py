...
def embedding_optimizer_factory(
    optimizer, objective, n_trials, n_dim, with_count, embedding, inverse
):
    ...
    best_val, best_v, feval_count = optimizer(
        objective=_objective, n_trials=n_trials, n_dim=m_dim, with_count=True
    )
...
def curl_factory(optimizer, objective, n_trials, n_dim, with_count, d):
    ...
    ranking = importance(
        objective, n_dim
    )  # Wastes n_dim+1 trials at least, so pretty stupid
    ...
    return embedding_optimizer_factory(
        optimizer=optimizer,
        objective=objective,
        n_trials=n_trials - 2 * n_dim - 2,
        n_dim=n_dim,
...
if __name__ == "__main__":
    ...
    best_val, best_x, feval_count = dlib_cube(
        objective=an_objective, n_trials=50, n_dim=14, with_count=True
    )
    best_val_0, best_x_0, feval_count_0 = dlib_curl_cube3(
        objective=an_objective, n_trials=150, n_dim=14, with_count=True
    )
    pass
    # The second version works in higher dimensions
    best_val_2, best_x_2, feval_count_2 = dlib_curl_cube3(
        objective=an_objective, n_trials=150, n_dim=34, with_count=True
    )
