...
def simplex_surf(g):
    ...
    X = np.array(
        [s1 * p1[0] + s2 * p2[0] + s3 * p3[0] for s1, s2, s3 in zip(s1s, s2s, s3s)]
    )
    Y = np.array(
        [s1 * p1[1] + s2 * p2[1] + s3 * p3[1] for s1, s2, s3 in zip(s1s, s2s, s3s)]
    )
    ...
    _surf = ax.plot_trisurf(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    _shadow = ax.plot_trisurf(
        X, Y, np.zeros_like(Z), cmap=cm.coolwarm, linewidth=0, antialiased=False
    )
    plt.show()
