    ...
    if sigma_scenarios is None:
        ...
        sigma_scenarios = (
            ...
            * np.random.multivariate_normal(
                mu, sigma_matrix, N_SCENARIOS, check_valid="warn", tol=1e-8
            )
        )
    ...
    if adjusted_scenarios is None:
        ...
        if False:  # Later...,
            ...
            diagonals = np.diagonal(
                np.dot(np.transpose(realized_sigma), realized_sigma)
            ).copy()
...
PORTFOLIO_OBJECTIVES = [
    markowitz_analytic_on_cube,
    markowitz_realized_on_cube,
    markowitz_return_on_cube,
    markowitz_skew_on_cube,
]
