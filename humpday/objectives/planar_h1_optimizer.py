if False:
    ...
    def best_locations(ws, offset, random_ws):
        ...
        def mb(zs):
            return locations_objective(
                zs=np.array(zs), random_ws=random_ws, offset=offset
            )
    ...
    if __name__ == "__main__":
        ...
        f = lambda x1, x2: hump_selector(
            xs=np.array([x1, x2]), zs=zBest, ws=random.choice(random_ws), offset=offset
        )
        mesh2d(f)
        pass
