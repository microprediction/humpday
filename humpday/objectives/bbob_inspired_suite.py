...
class BBOBInspiredSuite:
    ...
    def ellipsoid_separable(self, conditioning: float = 100.0, seed: int = 42) -> Callable:
    ...
    def ellipsoid_rotated(self, conditioning: float = 100.0, seed: int = 42) -> Callable:
    ...
    def rastrigin_rotated(self, seed: int = 42) -> Callable:
        ...
        def func(x):
            ...
            scaled_x = 10.24 * x - 5.12 + shift
            rotated_x = rotation_matrix @ scaled_x
            return 10 * len(rotated_x) + np.sum(rotated_x**2 - 10 * np.cos(2 * np.pi * rotated_x))
    ...
    def weierstrass(self, seed: int = 42) -> Callable:
        ...
        def func(x):
            ...
            constant = len(scaled_x) * sum(a**k * np.cos(np.pi * b**k) for k in range(kmax))
    ...
    def composition_function(self, base_functions: List[str], weights: List[float] = None, seed: int = 42) -> Callable:
    ...
    def get_systematic_suite(self) -> Dict[str, Callable]:
        ...
        suite = {
            ...
            'f02_ellipsoid_sep_moderate': self.ellipsoid_separable(conditioning=10, seed=2),
            ...
            'f06_ellipsoid_rotated_moderate': self.ellipsoid_rotated(conditioning=10, seed=6),
            'f07_rosenbrock_rotated': self.rosenbrock_rotated(seed=7),

            # High conditioning
            'f08_ellipsoid_rotated_ill': self.ellipsoid_rotated(conditioning=1000, seed=8),
    ...
    def get_function_metadata(self, function_name: str) -> Dict[str, Any]:
        ...
        return metadata_map.get(function_name, {
            'landscape_type': 'multimodal',
            'modality': 'multimodal',
            'separable': False,
            'conditioning': 'moderate',
...
if __name__ == "__main__":
    ...
    for name, func in functions.items():
        try:
            ...
            print(f"    {metadata.get('bbob_category', 'unknown'):15s} | "
    ...
    print(f"✓ Separable → Non-separable progression")
    print(f"✓ Well-conditioned → Ill-conditioned progression")
    print(f"✓ Unimodal → Multimodal progression")
    print(f"✓ Strong → Weak global structure progression")
    print(f"✓ Perfect for rigorous 3D Thurstone analysis")