...
dimension. We give the exact planar placement; across the full
HumpDay catalog, however, a greedy off-line placement does not beat a
line search, which locates the reduction's value in a fitted subspace
...
a plane, exactly, in any ambient dimension. Whether that exact
reduction pays is a separate question, and we test it across the
catalog: a greedy off-line placement does not beat a line search, which
locates where the reduction's value can be captured and where it
cannot.
...
equilateral apex over the base in a plane containing the line, with the
transverse direction either a random perpendicular or a running
momentum estimate of the improving direction. Start point, direction
stream, and adaptive step are identical, so the comparison is on-line
against off-line placement at the same budget. We run it across the
whole catalog: $51$ analytic objectives at $d\in\{2,5,10\}$ and $69$
physics simulators at their native dimensions, $222$ tasks in all.
...
placement beats the on-line one, over $222$ tasks ($51$ analytic
objectives and $69$ physics simulators) at $12$ to $16$ seeds each.
\emph{Random plane}: a random perpendicular transverse direction.
\emph{Momentum plane}: a running estimate of the improving direction,
projected off the line. The rate is below one half at every dimension.}
\label{tab:planar}
\end{table}

The verdict is a clean negative (Table~\ref{tab:planar}). The off-line
placement wins on about a quarter of seeds overall, and on no dimension
band does it reach one half; the momentum-informed transverse direction
is no better than a random one. Only \texttt{rastrigin} at $d=2$ shows
a consistent edge, $12$ of $16$ seeds; across the catalog the
fixed-geometry off-line move loses to staying on the line.

The reason is that the reduction is a fact about the \emph{posterior},
not about any particular objective. Under the prior the plane holds the
whole next-point decision, but a random or roughly-estimated transverse
direction almost never hits the objective's actual structure, so the
one off-line probe is usually spent on an uninformative direction.
Capturing the plane's value therefore needs a genuinely good direction
--- a fitted subspace model, as in LineBO --- rather than a cheap
greedy placement. Whether such a model turns the exact reduction into a
win is the open question this leaves.
