...
\begin{tabular}{lccc}
\toprule
dimension & planar beats line & multimodal & unimodal \\
\midrule
$d=2$  & $0.55$ & $0.56$ & $0.52$ \\
$d=5$  & $0.28$ & --- & --- \\
$d=10$ & $0.28$ & --- & --- \\
...
objectives. In two dimensions the transverse direction spans the rest
of the space and the off-line move mildly helps; in higher dimensions a
\emph{random} perpendicular is almost never the objective's informative
direction, and the move is wasted.}
\label{tab:planar}
\end{table}

The reduction is exact for the posterior, but exploiting it is not free
(Table~\ref{tab:planar}). In two dimensions the off-line placement wins
on $55\%$ of seeds; by five dimensions it wins on $28\%$, no better than
before. The gap is the isotropy assumption cashing out: any perpendicular
is equivalent \emph{under the prior}, but a random perpendicular is
almost surely not where the objective's structure lies, so in high
dimension the off-line evaluation is spent on an uninformative direction.
The exception proves the reading --- on \texttt{drop\_wave}, a radially
symmetric ripple, the off-line move wins on $16$--$17$ of $24$ seeds in
every dimension, because there the transverse direction always carries
signal. Using the plane in high dimension therefore needs an
\emph{informed} transverse direction, not a random one, which is where
LineBO's adaptive subspaces enter.
